...
contract Vault is IVault, Pausable, AccessControl {
    ...
    // LP tokens
    mapping(address => ILPToken) public supportedTokenToLPToken;
    mapping(address => address) public lpTokenToSupportedToken;
    ...
    constructor(
        ...
        Utils.CheckIsZeroAddress(_custodian);
        ...
        emit UpdateCustodian(address(0), _custodian);
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            ILPToken tokenTemp = ILPToken(_lpTokens[i]);
            supportedTokenToLPToken[token] = tokenTemp;
            lpTokenToSupportedToken[address(tokenTemp)] = token;
            emit LPTokenCreated(address(tokenTemp));
    ...
    function stake(address _token,  uint256 _stakedAmount) external onlySupportedToken(_token) whenNotPaused notBlacklisted(msg.sender) {
        ...
        supportedTokenToLPToken[_token].mint(msg.sender, mintAmount);
    ...
    function requestClaim(
        ...
        uint256 lpBalance = supportedTokenToLPToken[_token].balanceOf(msg.sender);

        if(sharesToBurn > lpBalance || assetsInfo.stakedAmount == 0) sharesToBurn = lpBalance;

        supportedTokenToLPToken[_token].burn(msg.sender, sharesToBurn);
    ...
    function _getExchangeRate(address _token) internal view returns (uint256 exchangeRate) {
        ...
        if (totalSupplyLPToken == 0) {
            ...
            exchangeRate = ((totalStakeAmountByToken[_token] + totalRewardsAmountByToken[_token]) * 1e18) / totalSupplyLPToken;
        }
    }
    ...
    function convertToShares(uint256 tokenAmount, address _token) public view returns (uint256 shares) {
        ...
        uint256 exchangeRate = totalSupplyLPToken == 0 ? 
        1e18 : (totalStaked + totalRewards) * 1e18 / totalSupplyLPToken;
    ...
    function transferOrTransferFrom(address token, address from, address to, uint256 amount) public notBlacklisted(msg.sender) notBlacklisted(from) notBlacklisted(to) returns (bool) {
        ...
        uint256 tokenBefore = getLPTokenAmount(from, token);
        require(tokenBefore >= amount, "balance");
        if(msg.sender != from){
            require(supportedTokenToLPToken[token].allowance(from, msg.sender) >= amount, "allowance");
            supportedTokenToLPToken[token].updateAllowance(from, msg.sender, amount);
            supportedTokenToLPToken[token].transferFrom(from, to, amount);
        }else{
            supportedTokenToLPToken[token].transferFrom(msg.sender, to, amount);
        }
    ...
    function transferToCustodian(
        ...
        IERC20(_token).safeTransfer(custodian, _amount);

        emit CustodianReceive(_token, custodian, _amount);
    }
    ...
    function addSupportedToken(
        ...
        require(_rewardRate < BASE && Utils.MustGreaterThanZero(_rewardRate), "Invalid reward rate");
        ...
        ILPToken tokenTemp = ILPToken(_lpToken);
        supportedTokenToLPToken[_token] = tokenTemp;
        lpTokenToSupportedToken[address(tokenTemp)] = _token;
        emit LPTokenCreated(address(tokenTemp));
        ...
        emit UpdateRewardRate(_token, 0, _rewardRate);
    }
    ...
    function setCustodian(address _newCustodian) external onlyRole(DEFAULT_ADMIN_ROLE) {
        Utils.CheckIsZeroAddress(_newCustodian);
        require(_newCustodian != custodian);

        emit UpdateCustodian(custodian, _newCustodian);
    ...
    function getLPTokenAmount(address _user, address _token) public view onlySupportedToken(_token) returns (uint256) {
        return supportedTokenToLPToken[_token].balanceOf(_user);
    }
